...
class TransferNet(nn.Module):
    def __init__(self, num_class, base_net='resnet50', transfer_loss='mmd', use_bottleneck=True, bottleneck_width=256, max_iter=1000, **kwargs):
        ...
        domain_loss_args = {
            "loss_type": self.domain_loss,
            "max_iter": max_iter,
            "num_class": num_class
        }
        self.adapt_loss = TransferLoss(**transfer_loss_args)
        self.domain_loss = TransferLoss(**domain_loss_args)
    ...
    def forward(self, source, target, source_label):
        ...
        # domain
        domain_loss = self.domain_loss(source, target, **{})

        return clf_loss, transfer_loss, domain_loss
    
    def get_parameters(self, initial_lr=1.0):
        params = [
            {'params': self.base_network.parameters(), 'lr': 1.5 * initial_lr},
        ...
        if self.domain_loss is not None:
            params.append(
                {'params': self.domain_loss.loss_func.domain_classifier.parameters(), 'lr': 1.0 * initial_lr}
            )
